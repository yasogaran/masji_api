...
async function main() {
  ...
  let totalHouses = 0;
  let totalFamilies = 0;
  const createdFamilyHeads: any[] = []; // Store family heads for later use
    ...
    for (let h = 0; h < housesPerMahalla; h++) {
      ...
      for (let f = 0; f < familiesInHouse; f++) {
        ...
        for (let p = 0; p < familySize; p++) {
          ...
          if (p === 1) relationshipTypeId = 2; // Spouse
          else if (p > 1) relationshipTypeId = isMale ? 3 : 4; // Son or Daughter
