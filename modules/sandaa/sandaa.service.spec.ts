...
describe('SandaaService', () => {
  ...
  beforeEach(async () => {
    ...
    service = module.get<SandaaService>(SandaaService);
  ...
  describe('getConfigs', () => {
    it('should return all configs', async () => {
      const mockConfigs = [
        { id: '1', amount: new Decimal(500), mahallaId: null, mahalla: null },
        { id: '2', amount: new Decimal(600), mahallaId: 'm1', mahalla: { title: 'North Area' } },
      ];
  ...
  describe('getActiveConfig', () => {
    ...
    it('should fall back to global config when no mahalla-specific config', async () => {
      ...
      mockPrismaService.sandaaConfig.findFirst
        .mockResolvedValueOnce(null) // No mahalla-specific config
        .mockResolvedValueOnce(mockGlobalConfig); // Global config
  ...
  describe('generatePayments', () => {
    it('should generate payments for eligible families', async () => {
      ...
      const mockConfig = { id: 'c1', amount: new Decimal(500) };

      mockPrismaService.person.findMany.mockResolvedValue(mockFamilyHeads);
      mockPrismaService.sandaaConfig.findFirst.mockResolvedValue(mockConfig);
      mockPrismaService.sandaaPayment.findUnique.mockResolvedValue(null); // No existing payment
  ...
  describe('getPayments', () => {
    it('should return paginated payments', async () => {
      const mockPayments = [
        { id: 'pay1', amount: new Decimal(500), status: 'pending' },
        { id: 'pay2', amount: new Decimal(500), status: 'paid' },
      ];
    ...
    it('should filter by status', async () => {
      ...
      expect(mockPrismaService.sandaaPayment.findMany).toHaveBeenCalledWith(
  ...
  describe('waivePayment', () => {
    it('should waive a payment', async () => {
      const mockPayment = { id: 'pay1', amount: new Decimal(500) };
  ...
  describe('getPendingPaymentsForFamily', () => {
    it('should get pending payments for a family head', async () => {
      ...
      const mockPendingPayments = [
        { id: 'pay1', amount: new Decimal(500), periodMonth: 1, periodYear: 2025 },
        { id: 'pay2', amount: new Decimal(500), periodMonth: 2, periodYear: 2025 },
      ];
  ...
  describe('getPaymentSummary', () => {
    it('should return payment summary', async () => {
      mockPrismaService.sandaaPayment.count
        ...
        .mockResolvedValueOnce(2); // partial
  ...
  describe('getAllConfigsByMahalla', () => {
    it('should return configs for all mahallas', async () => {
      ...
      const mockGlobalConfig = { id: 'c1', amount: new Decimal(500) };

      mockPrismaService.mahalla.findMany.mockResolvedValue(mockMahallas);
      mockPrismaService.sandaaConfig.findFirst
        .mockResolvedValueOnce(mockGlobalConfig) // Global config (currently effective)
        .mockResolvedValueOnce(null) // Pending global config
        .mockResolvedValueOnce(null) // m1 - no specific config
        .mockResolvedValueOnce(null) // m1 - no pending config
        .mockResolvedValueOnce({ id: 'c2', amount: new Decimal(600) }) // m2 - has config
        .mockResolvedValueOnce(null); // m2 - no pending config

      const result = await service.getAllConfigsByMahalla();

      expect(result).toHaveLength(3); // Global + 2 mahallas
      expect(result[0].mahallaTitle).toBe('Default (All Mahallas)');
      expect(result[1].isUsingGlobal).toBe(true);
      expect(result[2].isUsingGlobal).toBe(false); // m2 has its own config, not using global
    });
  });
  ...
  describe('recordCustomPayment', () => {
    it('should record custom payment across multiple months', async () => {
      ...
      mockPrismaService.sandaaPayment.findMany
        .mockResolvedValueOnce(mockPendingPayments) // Initial fetch
        .mockResolvedValueOnce([]); // After payment
      mockPrismaService.sandaaPayment.update.mockResolvedValue({});
