...
describe('MeetingsService', () => {
  ...
  describe('addDecision', () => {
    it('should add a decision to a meeting', async () => {
      ...
      expect(mockPrisma.meetingDecision.create).toHaveBeenCalledWith(
        expect.objectContaining({
          data: expect.objectContaining({
            meetingId: '1',
            decision: 'New decision',
    ...
    it('should throw BadRequestException if related issue not found', async () => {
      ...
      await expect(service.addDecision('1', decisionDto)).rejects.toThrow(
        BadRequestException
      );
    });
  });
  ...
  describe('removeDecision', () => {
    ...
    it('should throw NotFoundException if decision not found', async () => {
      ...
      await expect(service.removeDecision('nonexistent')).rejects.toThrow(
        NotFoundException
      );
    });
  });
  ...
  describe('getSummary', () => {
    it('should return meetings summary', async () => {
      mockPrisma.meeting.count
        ...
        .mockResolvedValueOnce(5); // recent (last 30 days)
      mockPrisma.meeting.findMany.mockResolvedValue([]);
    ...
    it('should return meetings for a specific month', async () => {
      ...
      expect(mockPrisma.meeting.findMany).toHaveBeenCalledWith(
        expect.objectContaining({
          where: expect.objectContaining({
            meetingDate: {
              gte: new Date(2024, 11, 1), // Dec 1, 2024
              lte: new Date(2024, 12, 0), // Dec 31, 2024
            },
          }),
        })
      );
