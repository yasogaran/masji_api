...
export class ReportsService {
  ...
  async getFinancialSummary(year: number) {
    ...
    const [incomes, expenses, otherPaymentIncome, otherPaymentExpense] = await Promise.all([
      // Regular income from Income model
      ...
      // Incoming payments from OtherPayment (paid only)
      ...
      // Outgoing payments from OtherPayment (paid only)
      this.prisma.otherPayment.aggregate({
        where: {
          status: 'paid',
          paidAt: {
  ...
  private async getMonthlyPaymentsData(year: number) {
  ...
  private async getMonthlyFinanceData(year: number) {
    const months = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'];
    
    const [incomeData, expenseData] = await Promise.all([
      this.prisma.$queryRaw<{ month: number; total: number }[]>`
  ...
  private async getPopulationByMahalla() {
  ...
  private async getRecentDonationsData() {
    const sixMonthsAgo = new Date();
    sixMonthsAgo.setMonth(sixMonthsAgo.getMonth() - 6);

    const donations = await this.prisma.$queryRaw<{ month: string; money: number; goods: number }[]>`
      SELECT
  ...
  private async getPaymentTrendsData(year: number) {
