...
/**
 * SandaaService - Works with Person-based families
 * 
 * In the new schema:
 ...
 */
@Injectable()
export class SandaaService {
  constructor(private prisma: PrismaService) {}

  // ============ CONFIG METHODS ============

  async getConfigs(mahallaId?: string) {
    const where: any = {};
    if (mahallaId) {
  ...
  async getActiveConfig(mahallaId?: string): Promise<any> {
    const today = new Date();
    return this.getConfigForDate(mahallaId, today);
  }

  /**
   * Get config that is effective for a specific date
   * This is useful when generating payments for future months
   */
  async getConfigForDate(mahallaId?: string, targetDate?: Date): Promise<any> {
    const date = targetDate || new Date();
    
    // First try to find mahalla-specific config
    if (mahallaId) {
  ...
  /**
   * Get config that is effective for a specific month/year period
   * Uses the first day of that month to determine which config is active
   */
  async getConfigForPeriod(mahallaId: string | undefined, month: number, year: number): Promise<any> {
    // Use first day of the period month to determine which config is active
    const periodDate = new Date(year, month - 1, 1);
    return this.getConfigForDate(mahallaId, periodDate);
  }

  async createConfig(dto: CreateSandaaConfigDto, createdBy?: string) {
    // Validate that there's no overlapping config for the same mahalla
  ...
  async updateConfig(id: string, dto: UpdateSandaaConfigDto) {
    const config = await this.prisma.sandaaConfig.findUnique({
      where: { id },
    });

    if (!config) {
      throw new NotFoundException('Sandaa config not found');
    }
  ...
  async generatePayments(dto: GeneratePaymentsDto, createdBy?: string) {
    const { month, year, mahallaId } = dto;

    // Get eligible family heads (familyHeadId = null AND isSandaaEligible = true)
    const familyHeadWhere: any = {
    ...
    for (const familyHead of familyHeads) {
      // Get the applicable config for this family's mahalla FOR THE PERIOD being generated
      // This ensures that if a new rate is scheduled for February, generating Feb payments uses the new rate
  ...
  async getPayments(query: SandaaPaymentQueryDto) {
    ...
    // familyId is now the family head's person ID
    if (familyId) {
  ...
  async getPaymentSummary(month: number, year: number, mahallaId?: string) {
    const where: any = {
      periodMonth: month,
      periodYear: year,
    };
    ...
    const expectedAmount = total._sum.amount || new Decimal(0);
    const collectedAmount = total._sum.paidAmount || new Decimal(0);
  ...
  async getYearlySummary(year: number, mahallaId?: string) {
    const where: any = {
      periodYear: year,
    };
    ...
    const expectedAmount = result._sum.amount || new Decimal(0);
    const collectedAmount = result._sum.paidAmount || new Decimal(0);
  ...
  async bulkRecordPayments(paymentIds: string[], receivedBy?: string) {
    const results = [];
  ...
  async waivePayment(paymentId: string, reason: string) {
    const payment = await this.prisma.sandaaPayment.findUnique({
      where: { id: paymentId },
    });
  ...
  async updateFamilyEligibility(dto: UpdateFamilyEligibilityDto) {
    // familyId is now the family head's person ID
    ...
    if (familyHead.familyHeadId !== null) {
      throw new BadRequestException('This person is not a family head');
    }
  ...
  async getEligibleFamilies(mahallaId?: string) {
    const where: any = {
      familyHeadId: null, // Family heads
      isSandaaEligible: true,
    };
  ...
  async getFamilyPaymentHistory(familyHeadId: string) {
  ...
  async checkPaymentsGenerated(month: number, year: number, mahallaId?: string) {
    const where: any = {
      periodMonth: month,
      periodYear: year,
    };
    ...
    // Get total eligible family heads for comparison
    const familyHeadWhere: any = {
      familyHeadId: null,
      isSandaaEligible: true,
    };
  ...
  // ============ GET PENDING PAYMENTS FOR FAMILY ============

  async getPendingPaymentsForFamily(familyHeadId: string) {
    const person = await this.prisma.person.findUnique({
      where: { id: familyHeadId },
      include: {
        house: { include: { mahalla: true } },
    ...
    // Get all pending payments for this family head
  ...
  async getAllConfigsByMahalla() {
    ...
    // Get global config (currently effective)
    ...
    // Get pending global config (will be effective in the future)
    ...
    for (const mahalla of mahallas) {
      // Get currently effective config for this mahalla
      const mahallaConfig = await this.prisma.sandaaConfig.findFirst({
        where: {
      ...
      // Get pending config for this mahalla (will be effective in the future)
  ...
  async getGenerationStatus(mahallaId?: string) {
    const where: any = {};
    if (mahallaId) {
  ...
  // ============ GENERATE PAYMENTS UNTIL PREVIOUS MONTH ============

  async generatePaymentsUntilPreviousMonth(mahallaId?: string, createdBy?: string) {
    ...
    let totalCreated = 0;
    let totalSkipped = 0;
    const monthsGenerated: string[] = [];

    let currentGenMonth = startMonth;
    let currentGenYear = startYear;

    while (
      currentGenYear < endYear ||
      (currentGenYear === endYear && currentGenMonth <= endMonth)
    ) {
      const result = await this.generatePayments(
        { month: currentGenMonth, year: currentGenYear, mahallaId },
        createdBy
      );
  ...
  // ============ GET CONSOLIDATED FAMILY PAYMENTS ============
  ...
  async getConsolidatedFamilyPayments(query: {
    ...
    // Build where clause for family heads
    const familyHeadWhere: any = {
      familyHeadId: null,
      isSandaaEligible: true,
    };
    ...
    // Get family heads with pagination
    ...
    // Get payment info for each family head
    const consolidatedData = await Promise.all(
      familyHeads.map(async (head) => {
        const payments = await this.prisma.sandaaPayment.findMany({
        ...
        const totalPendingAmount = outstandingPayments.reduce(
          (sum, p) => {
        ...
        const totalPaidAmount = payments.reduce(
          (sum, p) => sum + (p.paidAmount?.toNumber() || 0),
          0
        );

        let status: 'paid' | 'pending' | 'partial' | 'none' = 'none';
    ...
    // Calculate summary from ALL families (not just current page)
    ...
    const familyPaymentMap = new Map<string, { hasOutstanding: boolean; totalPending: number; totalPaid: number }>();
  ...
  // ============ RECORD PARTIAL/CUSTOM AMOUNT PAYMENT ============

  async recordCustomPayment(
    familyHeadId: string,
    amount: number,
    receivedBy?: string,
    notes?: string,
    ...
    if (outstandingPayments.length === 0) {
      throw new BadRequestException('No pending payments found for this family');
    }
  ...
  // ============ GET FAMILY COUNTS (ALL vs ELIGIBLE) ============
  async getFamilyCounts(mahallaId?: string) {
    const baseWhere: any = {
      familyHeadId: null,
    };
