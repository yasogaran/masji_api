...
describe('PaymentsService', () => {
  ...
  describe('getPaymentTypes', () => {
    it('should return active payment types by default', async () => {
      const mockTypes = [
        { id: '1', name: 'Building Fund', amount: new Decimal(1000), isActive: true, _count: { payments: 5 } },
        { id: '2', name: 'Charity', amount: new Decimal(500), isActive: true, _count: { payments: 3 } },
      ];
  ...
  describe('updatePaymentType', () => {
    it('should update an existing payment type', async () => {
      const existingType = { id: '1', name: 'Old Name', amount: new Decimal(500) };
      const updateDto = { name: 'New Name', amount: 600 };

      mockPrismaService.paymentType.findUnique
        .mockResolvedValueOnce(existingType) // First call - check if exists
  ...
  describe('getPayments', () => {
    it('should return paginated payments', async () => {
      const mockPayments = [
        { id: 'p1', amount: new Decimal(500), status: 'pending' },
        { id: 'p2', amount: new Decimal(1000), status: 'paid' },
      ];
    ...
    it('should filter by status', async () => {
      ...
      expect(mockPrismaService.otherPayment.findMany).toHaveBeenCalledWith(
  ...
  describe('createPayment', () => {
    it('should create a new payment', async () => {
      ...
      const mockPaymentType = { id: 'type-1', name: 'Building Fund', amount: new Decimal(500), isActive: true };
    ...
    it('should use default amount from payment type if not provided', async () => {
      ...
      const mockPaymentType = { id: 'type-1', amount: new Decimal(750), isActive: true };
      ...
      expect(mockPrismaService.otherPayment.create).toHaveBeenCalledWith(
        expect.objectContaining({
  ...
  describe('recordPayment', () => {
    it('should record a payment as paid', async () => {
      const mockPayment = { id: 'p1', status: 'pending', amount: new Decimal(500) };
  ...
  describe('getPersonPayments', () => {
    it('should return payments for a person', async () => {
      const mockPerson = { id: 'person-1', fullName: 'John' };
      const mockPayments = [
        { id: 'p1', amount: new Decimal(500), paymentType: { name: 'Type A' } },
        { id: 'p2', amount: new Decimal(300), paymentType: { name: 'Type B' } },
      ];
  ...
  describe('getPaymentSummary', () => {
    it('should return payment summary', async () => {
      mockPrismaService.otherPayment.aggregate
        .mockResolvedValueOnce({ _count: 5, _sum: { amount: new Decimal(2500) } }) // pending
        .mockResolvedValueOnce({ _count: 10, _sum: { amount: new Decimal(5000) } }) // paid
        .mockResolvedValueOnce({ _count: 17, _sum: { amount: new Decimal(7500) } }) // total
        .mockResolvedValueOnce({ _count: 8, _sum: { amount: new Decimal(4000) } }) // incomingPaid
        .mockResolvedValueOnce({ _count: 2, _sum: { amount: new Decimal(1000) } }); // outgoingPaid
      mockPrismaService.otherPayment.count.mockResolvedValue(2); // cancelled
    ...
    it('should filter summary by payment type', async () => {
      mockPrismaService.otherPayment.aggregate.mockResolvedValue({ _count: 0, _sum: { amount: null } }); // used for all 5 aggregate calls
      mockPrismaService.otherPayment.count.mockResolvedValue(0);
