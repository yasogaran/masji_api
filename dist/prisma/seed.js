        ...
        for (const mahalla of mahallas) {
            ...
            for (let h = 0; h < housesPerMahalla; h++) {
                ...
                for (let f = 0; f < familiesInHouse; f++) {
                    const familySize = 3 + Math.floor(Math.random() * 3);
                    let familyHead = null;
                    totalFamilies++;
                    for (let p = 0; p < familySize; p++) {
                        ...
                        let relationshipTypeId = 1;
                        if (p === 1)
                            relationshipTypeId = 2;
                        else if (p > 1)
                            relationshipTypeId = isMale ? 3 : 4;
                        ...
                        const person = await prisma.person.create({
                            data: {
                                ...
                                houseId: house.id,
                                familyHeadId: p > 0 ? familyHead?.id : null,
                                relationshipTypeId: relationshipTypeId,
                                memberStatusId: 1,
                                civilStatusId: p < 2 ? 2 : (age > 18 ? 1 : null),
