...
describe('ZakathService', () => {
    ...
    beforeEach(async () => {
        ...
        const module = await testing_1.Test.createTestingModule({
            providers: [
                zakath_service_1.ZakathService,
                {
        ...
        service = module.get(zakath_service_1.ZakathService);
    });
    afterEach(() => {
        jest.clearAllMocks();
    });
    describe('Categories', () => {
        ...
        it('should throw when category not found', async () => {
            ...
            await expect(service.getCategoryById('nonexistent')).rejects.toThrow(common_1.NotFoundException);
        });
        it('should not delete category used in requests', async () => {
            ...
            await expect(service.deleteCategory('cat1')).rejects.toThrow(common_1.BadRequestException);
        });
    });
    describe('Periods/Cycles', () => {
        ...
        it('should complete a cycle with totals', async () => {
            ...
            mockPrismaService.zakathCollection.aggregate.mockResolvedValue({ _sum: { amount: new library_1.Decimal(50000) } });
            mockPrismaService.zakathDistribution.aggregate.mockResolvedValue({ _sum: { amount: new library_1.Decimal(45000) } });
            mockPrismaService.zakathPeriod.update.mockResolvedValue({
                ...mockPeriod,
                status: 'completed',
                totalCollected: new library_1.Decimal(50000),
                totalDistributed: new library_1.Decimal(45000),
            });
        ...
        it('should throw when completing already completed cycle', async () => {
            ...
            await expect(service.completeCycle('period1')).rejects.toThrow(common_1.BadRequestException);
        });
        it('should not delete period with collections', async () => {
            mockPrismaService.zakathPeriod.findUnique.mockResolvedValue(mockPeriod);
            await expect(service.deletePeriod('period1')).rejects.toThrow(common_1.BadRequestException);
        });
    });
    describe('Collections', () => {
        const mockCollection = {
            ...
            donorId: 'person1',
            amount: new library_1.Decimal(10000),
    ...
    describe('Requests', () => {
        const mockRequest = {
            ...
            requesterId: 'person1',
            categoryId: 'cat1',
            amountRequested: new library_1.Decimal(5000),
        ...
        it('should approve a request', async () => {
            ...
            mockPrismaService.zakathRequest.update.mockResolvedValue({
                ...mockRequest,
                status: 'approved',
                amountApproved: new library_1.Decimal(4500),
            });
        ...
        it('should not approve non-pending request', async () => {
            ...
            await expect(service.approveRequest('req1', { amountApproved: 4500 })).rejects.toThrow(common_1.BadRequestException);
        });
    });
    describe('Distributions', () => {
        const mockApprovedRequest = {
            id: 'req1',
            status: 'approved',
            amountApproved: new library_1.Decimal(5000),
            distributions: [],
        };
        it('should create a distribution', async () => {
            ...
            mockPrismaService.zakathDistribution.create.mockResolvedValue({
                ...
                amount: new library_1.Decimal(5000),
            ...
            expect(result.amount).toEqual(new library_1.Decimal(5000));
        });
        it('should create partial distribution', async () => {
            ...
            mockPrismaService.zakathDistribution.create.mockResolvedValue({
                id: 'dist1',
                zakathRequestId: 'req1',
                amount: new library_1.Decimal(2500),
            });
            mockPrismaService.zakathRequest.update.mockResolvedValue({
                ...mockApprovedRequest,
                status: 'partial',
            ...
            expect(result.amount).toEqual(new library_1.Decimal(2500));
        });
        it('should throw when distribution exceeds approved amount', async () => {
            mockPrismaService.zakathRequest.findUnique.mockResolvedValue({
                ...mockApprovedRequest,
                distributions: [{ amount: new library_1.Decimal(3000) }],
            });
    ...
    describe('Reports', () => {
        ...
        it('should generate period report', async () => {
            ...
            mockPrismaService.zakathCollection.aggregate.mockResolvedValue({
                _sum: { amount: new library_1.Decimal(50000) },
                _count: 10,
            });
            mockPrismaService.zakathDistribution.aggregate.mockResolvedValue({
                _sum: { amount: new library_1.Decimal(40000) },
                _count: 8,
            });
            mockPrismaService.zakathRequest.groupBy.mockResolvedValue([
                { status: 'pending', _count: 2, _sum: { amountRequested: new library_1.Decimal(10000) } },
                { status: 'approved', _count: 3, _sum: { amountApproved: new library_1.Decimal(15000) } },
            ]);
            ...
            expect(result.summary.totalCollected).toEqual(new library_1.Decimal(50000));
            expect(result.summary.totalDistributed).toEqual(new library_1.Decimal(40000));
            expect(result.summary.balance).toBe(10000);
        });
        it('should generate overall report', async () => {
            mockPrismaService.zakathPeriod.findMany.mockResolvedValue([mockPeriod]);
            mockPrismaService.zakathCollection.aggregate.mockResolvedValue({
                _sum: { amount: new library_1.Decimal(100000) },
                _count: 50,
            });
            mockPrismaService.zakathDistribution.aggregate.mockResolvedValue({
                _sum: { amount: new library_1.Decimal(80000) },
                _count: 40,
            });
            mockPrismaService.zakathRequest.groupBy.mockResolvedValue([]);
            mockPrismaService.zakathCategory.findMany.mockResolvedValue([]);
            const result = await service.getOverallReport();
            expect(result.overall.totalCollected).toEqual(new library_1.Decimal(100000));
            expect(result.overall.totalDistributed).toEqual(new library_1.Decimal(80000));
        });
    });
});
//# sourceMappingURL=zakath.service.spec.js.map